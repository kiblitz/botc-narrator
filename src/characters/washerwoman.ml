      ...
      if Game_state.is_poisoned state pid then begin
        ...
        let tf_names =
          List.filter_map (Game_state.alive_players state) ~f:(fun p ->
            if Character_intf.is_townsfolk p.Player.character
            then Some (Character_intf.name p.Player.character)
            else None)
        in
        let* char_name = Narrator_monad.narrator_pick tf_names in
        ...
        let townsfolk =
          List.filter (Game_state.alive_players state)
            ~f:(fun p -> Character_intf.is_townsfolk p.Player.character)
        in
        let* (tf, _)    = Character.narrator_pick_from townsfolk in
        let other_ids   = Character.alive_except state tf.Player.id in
        let* (other, _) = Character.narrator_pick_from other_ids in
        let* (swap, _)  = Character.narrator_pick_from [false; true] in
        let p1, p2      = if swap then (tf.Player.id, other) else (other, tf.Player.id) in
        Narrator_monad.show_info pid
          (Narrator_monad.Two_players_one_char (p1, p2, Character_intf.name tf.Player.character))
      end
    in
